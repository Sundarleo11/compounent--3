...
class Search extends Component{
    ...
    getTweetsByUser = (event) => {
        axios
         ...
         // .get(`https://lnnp2qppv5.execute-api.us-east-1.amazonaws.com/tweetapp/api/v1.0/tweets/${this.username.current.value}`)
          .get(`https://w7eobia5cl.execute-api.us-east-1.amazonaws.com/FindById-v1/api/v1.0/tweets/${this.username.current.value}`)
         // console.log(this.username.current.value) 
         .then((res) => {

            this.dataArray = res.data;
            if(this.dataArray.length>0){
              this.setState({dataArray:res.data,null:""})
                
            }
            else if(this.dataArray.length === 0){
              this.setState({null:"No Tweets added so far or user not found",dataArray:""})
            }
